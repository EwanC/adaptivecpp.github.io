...
Khronos [VulkanHpp](https://github.com/KhronosGroup/Vulkan-Hpp) headers
are use that provide useful concepts like RAII wrapped handles.
...
Let's elaborate on why those extensions are necessary building blocks of our SYCL implementations.
...
it uses monotonically increasing 64-bit integer value to define synchronization order, eliminating the need to be reset
before reuse. What's more, a timeline semaphore can also be host or device which is useful for reasons we'll discuss later.
...
we needed to solve that was not achieved in Sylkan or any of the OpenCL-on-Vulkan layered implementations
...
There are three main pieces to assemble before you can compile your first SYCL program for Vulkan: the LunarG SDK,
a clspv binary, and AdaptiveCpp itself. The first main build dependency is the [LunarG Vulkan SDK](https://vulkan.lunarg.com/sdk/home)
...
With the Ubuntu setup working, let's look at what this enables on Android.
To illustrate the benefits of the SYCL acceleration enabled by AdaptiveCpp Android support we cross compiled one of the benchmarks from
...
the mandelbrot benchmark which has among others a OpenMP variant
...
Adreno 750 GPU and Arm v8a we achieved a 4.4x speedup over raw OpenMP.
...
The speed 2.8x speedup from the SYCL OpenMP backend over the straight OpenMP benchmark, is something that's
observable on an desktop platform and not only Android. It is due to the higher parallel execution times
...
This is just the beginning however, our roadmap of tasks is tracked as AdaptiveCpp GitHub issues under the
...
that could bring together SYCL and Vulkan graphics APIs together in the same application

If this work interest you please try it out and share your results, there's a lot of different Vulkan drivers and application
